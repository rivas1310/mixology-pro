...
export default function EditWinePage() {
  ...
  const loadWine = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        toast.error('Vino no encontrado')
      ...
      console.error(e)
      toast.error('Error cargando vino')
      router.push('/admin/wines')
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Error actualizando el vino')
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-2">
              Actualiza la información del vino
            </p>
          </div>
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Input
                  list="wine-type-suggestions-edit"
                  value={formData.type}
                  onChange={(e) => handleInputChange('type', e.target.value)}
                  placeholder="Ej: Premium, Reserva, Mesa, crianza, joven…"
                  required
                />
                <datalist id="wine-type-suggestions-edit">

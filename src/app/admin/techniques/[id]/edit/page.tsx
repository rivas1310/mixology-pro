...
export default function EditTechniquePage() {
  ...
  const loadTechnique = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          name: data.name || '',
          category: data.category || 'SHAKING',
        ...
        if (data.steps) {
          ...
          setSteps(Array.isArray(parsedSteps) && parsedSteps.length > 0 ? parsedSteps : [{ step: 1, instruction: '', tip: '' }])
        }
      }
    } catch (error) {
      console.error('Error loading technique:', error)
      toast.error('Error cargando técnica')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Técnica actualizada exitosamente')
        router.push('/admin/techniques')
      } else {
        const error = await response.json()
        toast.error(error.error || 'Error actualizando la técnica')
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-2">
              Modifica los detalles de la técnica
            </p>
          </div>
        </div>

...
export default function EditBeerPage() {
  ...
  const loadBeer = useCallback(async () => {
    try {
      const response = await fetch(`/api/admin/beers/${id}`)
      if (response.ok) {
        const data = await response.json()
        setFormData({
          name: data.name || '',
          brand: data.brand || '',
          type: data.type || 'CRAFT',
          category: data.category || 'LAGER',
      ...
      toast.error('Error cargando cerveza')
    } finally {
      setLoading(false)
    }
  }, [id])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Error actualizando la cerveza')
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-2">
              Actualiza la información de la cerveza
            </p>
          </div>
        </div>
